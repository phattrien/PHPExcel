...
<div class="control m-bottom">
    <div class="pull-left">{COUNT_PRODUCT}</div>
    <div class="pull-right">
        <a href="{URL_DOWNLOAD}" target="_blank" class="btn btn-primary btn-xs"><em class="fa fa-download">&nbsp;</em>{LANG.export}</a> <a href="{URL_IMPORT}" class="btn btn-primary btn-xs"><em class="fa fa-folder-o">&nbsp;</em>{LANG.import}</a>
    </div>
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="" title="{ROW.title}" class="open_modal" data-width="{ROW.imghome_info.width}" data-src="{ROW.imghome}"><img src="{ROW.thumb}" alt="{ROW.title}" width="40" /></a></td>
                    <td class="top">
                        <p>
...
<blockquote>
    <ul style="padding: 0">
        <li>{LANG.phpexcel_guide_1}</li>
        <li>{LANG.phpexcel_guide_2}</li>
        <li>{LANG.phpexcel_guide_3}</li>
    </ul>
</blockquote>
...
<div class="well">
    <form action="{ACTION}" id="frm-import" method="post" enctype="multipart/form-data">
        <div class="row">
            ...
            <div class="col-xs-20">
                <button type="submit" class="btn btn-primary btn-sm" id="btn-perform" disabled="disabled">
                    <em class="fa fa-check">&nbsp;</em>{LANG.perform}
                </button>
            </div>
        </div>
    </form>
...
<div id="msexcel-success" class="alert alert-success" style="display: none">{LANG.perform_success}</div>

<div id="msexcel-error" style="display: none">
    <div class="alert alert-danger">{LANG.phpexcel_error_note}</div>
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
            <thead>
                <tr>
                    <th class="w100 text-center">{LANG.phpexcel_id}</th>
                    <th class="w150">{LANG.phpexcel_listcatid}</th>
                    <th class="w150">{LANG.phpexcel_code}</th>
                    <th>{LANG.name}</th>
                    <th>{LANG.phpexcel_error}</th>
                </tr>
            </thead>
            <tbody>
		...
		success: function(json) {
			try{
				...
				$.each(json, function(index, value){
					...
					html += '<tr>';
					html += '	<td>' + value.id + '</td>';
					html += '	<td>' + value.cat + '</td>';
					html += '	<td>' + value.code + '</td>';
					html += '	<td>' + value.title + '</td>';
					html += '	<td>' + value.message + '</td>';
					html += '</tr>';
					$('#msexcel-error table tbody').html('').append(html);
				});
				$('#msexcel-error').show();
			}
			catch(err){
				if(json == 'OK'){
					$('#msexcel-success').show();
				}
			}       
		}
	});
